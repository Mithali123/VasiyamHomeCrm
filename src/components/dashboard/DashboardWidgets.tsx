    ...
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm flex flex-col overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto max-h-[350px]">
        {attentionQueue.map((item) => (
          <div key={item.id} className="p-4 border-b border-gray-50 last:border-0 hover:bg-gray-50 transition-colors group cursor-pointer">
            <div className="flex items-start gap-3">
              <div className={cn(
                "p-2 rounded-lg",
                item.severity === 'high' ? "bg-red-50 text-red-600" : 
                item.severity === 'medium' ? "bg-orange-50 text-orange-600" : "bg-blue-50 text-blue-600"
              )}>
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[10px] text-gray-500 mt-0.5">Lead: <span className="text-gray-900 font-semibold">{item.lead}</span></p>
              </div>
      ...
      <button 
        ...
        className="p-3 text-center text-xs font-bold text-primary hover:bg-primary/5 transition-colors border-t border-gray-100"
      >
        View All Tasks
      </button>
    </div>
          ...
          <div key={rep.name} className="flex items-center gap-3 group cursor-pointer">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-3 mt-1">
                <span className="text-[10px] text-gray-400">Visits: <b className="text-gray-900">{rep.visits}</b></span>
                <span className="text-[10px] text-gray-400">Bookings: <b className="text-gray-900">{rep.bookings}</b></span>
              </div>
            </div>
            <div className="text-right">
              <p className="text-xs font-bold text-green-600">{rep.rate}</p>
              <p className="text-[9px] text-gray-400 font-medium">Conv. Rate</p>
            </div>
          </div>
        ))}
      </div>
...
export function LeadSummaryWidget() {
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm flex flex-col overflow-hidden col-span-1 lg:col-span-2">
      <div className="p-5 border-b border-gray-100 flex items-center justify-between">
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-gray-50/50">
              <th className="px-5 py-3 text-[10px] font-bold text-gray-400 uppercase">Lead Name</th>
              <th className="px-5 py-3 text-[10px] font-bold text-gray-400 uppercase">Project</th>
              <th className="px-5 py-3 text-[10px] font-bold text-gray-400 uppercase">RM</th>
              <th className="px-5 py-3 text-[10px] font-bold text-gray-400 uppercase">Stage</th>
              <th className="px-5 py-3 text-[10px] font-bold text-gray-400 uppercase">Next Follow-up</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-50">
            {recentLeads.map((lead) => (
              <tr key={lead.name} className="hover:bg-gray-50 transition-colors cursor-pointer group">
                ...
                <td className="px-5 py-3 text-xs text-gray-600">{lead.project}</td>
                <td className="px-5 py-3 text-xs text-gray-600">{lead.rm}</td>
                <td className="px-5 py-3">
                  <span className={cn(
                    "text-[10px] px-2 py-0.5 rounded-full font-bold",
                    lead.stage === 'Qualified' ? "bg-green-100 text-green-700" :
                    lead.stage === 'New' ? "bg-blue-100 text-blue-700" :
                    lead.stage === 'Site Visit' ? "bg-purple-100 text-purple-700" : "bg-orange-100 text-orange-700"
                  )}>
                    {lead.stage}
                  </span>
                </td>
                <td className="px-5 py-3 text-xs font-semibold text-gray-900">{lead.next}</td>
              </tr>
            ))}
          </tbody>
        </table>
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm flex flex-col overflow-hidden">
      ...
      <div className="p-5 flex-1 relative">
        <div className="absolute left-7 top-5 bottom-5 w-px bg-gray-100" />
