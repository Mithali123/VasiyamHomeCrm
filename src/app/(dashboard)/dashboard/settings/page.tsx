...
const defaultProjects: Project[] = [
  { id: 1, name: "Vasiyam Pride", status: "Ongoing", location: "Medavakkam, Chennai", category: "Apartments" },
  { id: 2, name: "King's Garden", status: "Ongoing", location: "Tambaram, Chennai", category: "Independent Plots" },
  { id: 3, name: "BGR Garden", status: "Ongoing", location: "Chennai", category: "Villa Plots" },
  { id: 4, name: "Vasiyam Florence", status: "Ongoing", location: "Chennai", category: "Apartments" },
  { id: 5, name: "Grandeur", status: "Completed", location: "Medavakkam", category: "Premium Apartments" },
];
...
const Button = ({
  ...
  const variants = {
    primary: "bg-[#176044] text-white hover:bg-[#124e37]",
    secondary: "border border-[#dfe4e2] text-[#55605c] hover:bg-[#f4f6f5]",
  ...
  return (
    <button
      onClick={onClick}
      className={`inline-flex h-10 items-center justify-center gap-1.5 rounded-lg px-4 text-xs font-semibold transition ${variants[variant]} ${className}`}
    >
      {children}
    </button>
  );
...
export default function SettingsPage() {
  ...
  const addUser = () => {
    ...
    setUsers((current) =>
      editingUser === null
        ? [...current, { id: Date.now(), ...userForm, active: true }]
    ...
    notify(editingUser === null ? "User added successfully" : "User updated successfully");
  };
  ...
  const addProject = () => {
    ...
    setProjects((current) =>
      editingProject === null
        ? [...current, { id: Date.now(), ...projectForm }]
        : current.map((project) =>
            project.id === editingProject ? { id: project.id, ...projectForm } : project
          )
    );

    setProjectForm({ name: "", status: "Ongoing", location: "", category: "" });
    setEditingProject(null);
    setShowProjectForm(false);
    notify(editingProject === null ? "Project added successfully" : "Project updated successfully");
  };
            ...
            {active === "users" && (
              <>
                <PanelHeader
                  ...
                  description="Create, edit, activate and manage CRM users."
                />
                <div className="p-5 sm:p-6">
                  ...
                  {!showUserForm && (
                    <button
                      ...
                      className="mb-4 inline-flex h-10 items-center gap-1.5 rounded-lg bg-[#176044] px-4 text-xs font-semibold text-white hover:bg-[#124e37]"
                    >
                  ...
                  {/* User Form */}
                  {showUserForm && (
                    <div className="mb-4 rounded-xl border border-[#e4e9e7] bg-[#fafcfb] p-4">
                      <div className="grid gap-3 md:grid-cols-[1fr_1fr_190px_auto]">
                        ...
                        <div className="flex gap-2">
                          <Button onClick={addUser}>
                            {editingUser === null ? <Plus size={14} /> : <Save size={14} />}
                            {editingUser === null ? "Add User" : "Save Changes"}
                          </Button>
                          <Button variant="secondary" onClick={cancelUserForm}>
                            Cancel
                          </Button>
                        </div>
                      </div>
                            ...
                            <td className="px-3 py-3">
                              <div className="flex justify-end gap-1">
                                ...
                                <button
                                  ...
                                  onClick={() => notify(`Password reset sent to ${user.email}`)}
            ...
            {active === "roles" && (
              <>
                <PanelHeader
                  ...
                  description="Control module and feature access for each role."
                />
            ...
            {active === "leads" && (
              <>
                <PanelHeader
                  ...
                  description="Manage sources, statuses, pipeline stages and scoring rules."
                />
                <div className="grid gap-5 p-5 sm:p-6 xl:grid-cols-2">
                  <TagManager
                    title="Lead Sources"
                    description="Where leads are generated."
                    items={sources}
                    onChange={setSources}
                  />
                  <TagManager
                    title="Lead Status"
                    description="Available lead status values."
                    items={statuses}
                    onChange={setStatuses}
                  />
                  <TagManager
                    title="Pipeline Stages"
                    description="Stages used in the sales pipeline."
                    items={pipeline}
                    onChange={setPipeline}
                  />
                  <TagManager
                    title="Lead Score Rules"
                    description="Rules that increase or reduce lead scores."
                    items={scoreRules}
            ...
            {active === "projects" && (
              <>
                <PanelHeader
                  ...
                  description="Add and maintain project information used by the sales team."
                />
                <div className="p-5 sm:p-6">
                  {/* Add Project Button */}
                  {!showProjectForm && (
                  ...
                  {showProjectForm && (
                    <div className="mb-4 rounded-xl border border-[#e4e9e7] bg-[#fafcfb] p-4">
                      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-[1.2fr_.8fr_1fr_1fr_auto]">
                        <input
                        ...
                        <input
                          ...
                          placeholder="Category (e.g., Apartments)"
                          className={inputClass}
                        />
                        <div className="flex gap-2">
                          <Button onClick={addProject}>
                            {editingProject === null ? <Plus size={14} /> : <Save size={14} />}
                            {editingProject === null ? "Add Project" : "Save Changes"}
                          </Button>
                          <Button variant="secondary" onClick={cancelProjectForm}>
                            Cancel
                          </Button>
                        </div>
                      </div>
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full min-w-[720px] text-left text-xs">
                      <thead className="border-b border-[#dfe4e1] text-[10px] uppercase tracking-wide text-[#87908d]">
                        <tr>
                      ...
                      <tbody>
                        {projects.map((project) => (
                          <tr key={project.id} className="border-b border-[#edf0ee] last:border-0">
                            <td className="px-3 py-4 font-medium text-[#202622]">{project.name}</td>
                            <td className="px-3 py-4 text-[#59615c]">{project.location}</td>
                            <td className="px-3 py-4 text-[#59615c]">{project.category}</td>
            ...
            {active === "notifications" && (
              <>
                <PanelHeader
                  ...
                  description="Choose which alerts administrators and users receive."
                />
            ...
            {active === "system" && (
              <>
                <PanelHeader
                  ...
                  description="Configure company identity and regional preferences."
                />
                <div className="p-5 sm:p-6">
                  {/* Logo Upload */}
